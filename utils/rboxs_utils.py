...
def regular_theta(theta, mode='180', start=-pi/2):
    ...
    theta = theta % cycle
    return theta + start
#poly是四个点的坐标[x1, y2, x2, y2, x3, y3, x4, y4]，rbox是[xc, yc, w, h, theta]
...
def rbox2poly(obboxes):
    '''这段代码实现了将旋转框（rbox）表示转换为四边形（poly）表示的功能。
    具体来说，对于每个旋转框，它提取了中心点坐标、宽度、高度和旋转角度，
    然后通过这些信息计算出四个顶点的坐标，最终将这些坐标组合成四边形的表示形式。'''
    """
    Trans rbox format to poly format.
    Args:
...
def poly2hbb(polys):
    '''这段代码实现了将多边形（poly）表示转换为水平边界框（hbb）表示的功能。
    对于每个多边形，它提取了多边形的顶点坐标，并通过计算最小包围矩形的方式得到一个水平边界框。'''
    """
