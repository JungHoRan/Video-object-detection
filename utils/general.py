...
#obb特有
'''
clip:剪辑，修剪，将某个值限定在一定范围内
coords是指边界框的坐标
该函数用于将边界框的坐标限制在图像边界内'''
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    #判断每一位的置信度是否大于阈值，返回一个（1,18900）的bool型tensor
    ...
    #xi表示第几个batch x表示这个batch内所有的预测框
    for xi, x in enumerate(prediction):  # image index, image inference
        # Apply constraints
        # x[((x[..., 2:4] < min_wh) | (x[..., 2:4] > max_wh)).any(1), 4] = 0  # width-height
        # xi为0 表示第0个batch
        #xc[xi]表示获取的这个batch内所有的置信度是否大于阈值
        #x=x[xc[xi]]将x中为Ture的值取出，赋值给x
        #此时的x是第一步置信度筛选后剩下的预测框
        x = x[xc[xi]]  # confidence

        # Cat apriori labels if autolabelling
        #检查标签是否存在，并且第xi批数据对应的标签是否为空
        if labels and len(labels[xi]):
            l = labels[xi] #存储
            v = torch.zeros((len(l), nc + 5), device=x.device)#创建指定大小的张量
            # box将标签中的边界框坐标（左上角和右下角）复制到张量v的前四列中
            v[:, :4] = l[:, 1:5]
            v[:, 4] = 1.0  # conf，将张量v的第五列（置信度）设置为1，表示有物体
            #根据标签中的类别信息，将v中对应类别的概率设为1.0
            #此时的v:前五列是坐标和置信度，第六列开始往后nc列分别是对应各个类别的概率
            v[range(len(l)), l[:, 0].long() + 5] = 1.0  # cls
            #将x和v拼接起来，以便损失计算和反向传播
            x = torch.cat((x, v), 0)
        ...
        #判断是否参与多标签模式，没有的话就使用最高类别置信度的预测框
        if multi_label:
            ...
            '''将结果拼在一起
            box shape为（52,4），表示52个预测框的xyxy坐标值
            conf shape为（52,1），表示52个预测框的最大类别概率
            j shape为（52,1），表示最大类别概率的索引值，表示所属类别
            '''
        ...
        # Batched NMS
        #计算调整后的偏置量，如果agnostic为True,表示多个类一起nms,否则按照不同类分别进行计算
        c = x[:, 5:6] * (0 if agnostic else max_wh)  # classes
        boxes, scores = x[:, :4] + c, x[:, 4]  # boxes (offset by class), scores
        """
        其中boxes为原来的box加上c的偏置量，为了确保不同类别的预测框不会重叠
        ...
        if i.shape[0] > max_det:  # limit detections
            i = i[:max_det]
        #Merge NMS技术,将具有重叠的边界框合并成一个较大的边界框
        #如果要合并且框数在1到3000之间
...
def non_max_suppression_obb(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        #’_‘代表我们不关心的值，theta_pred就是置信度最高的角度值
        ...
        #此时的x变成了上述格式（中心点坐标，宽高，旋转角度，最高置信度（类别的而非角度的），最高置信度索引）
        if multi_label:
