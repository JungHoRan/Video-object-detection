...
def save_one_txt(predn, save_conf, shape, file):
    ...
    for *xyxy, conf, cls in predn.tolist():
        #将xyxy(左上角+右下角)格式转换成xywh(中心点+宽高)
        xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()  # normalized xywh
        #line的形式是：类别xywh,若save_conf为true,line就是类别、xywh、置信度
...
def save_one_json(pred_hbbn, pred_polyn, jdict, path, class_map):
    ...
    for p, b in zip(pred_polyn.tolist(), box.tolist()):
        jdict.append({'image_id': image_id,
                      ...
                      'bbox': [round(x, 1) for x in b],#预测框坐标
                      'score': round(p[-2], 5),#预测得分
...
def process_batch(detections, labels, iouv):
    ...
    if x[0].shape[0]:
        #将符合条件的位置构建成一个新的矩阵，第一列是行索引（gt索引），第二列是列索引（预测框索引），第三列是iou值
        ...
        if x[0].shape[0] > 1:
            ...
            '''
            上述一行代码实现，每个预测框最多只能出现一次，如果有一个预测框和多个gt匹配，只取其最大iou的一个'''
            # matches = matches[matches[:, 2].argsort()[::-1]]
            matches = matches[np.unique(matches[:, 0], return_index=True)[1]]
            '''上述一行代码实现，每个gt也最多出现一次，如果有一个gt同时和多个预测框匹配，只取匹配最大一个'''
        #实现了给每个gt分配一个iou最高的类别的预测框，一一对应
        matches = torch.Tensor(matches).to(iouv.device)
        correct[matches[:, 1].long()] = matches[:, 2:3] >= iouv
        #这里的matches[:,1]表示的是为对应的预测框来赋予其iou所能达到的最大程度
    return correct
...
def run(data,
    ...
    if training:  # called by train.py 通过train.py调用run函数
        device, pt, jit, engine = next(model.parameters()).device, True, False, False  # get model device, PyTorch model

        half &= device.type != 'cpu'  # half precision only supported on CUDA
        model.half() if half else model.float()
    else:  # called directly 通过val.py调用run函数
...
##3、加载配置
    # Configure
    #切换为测试模式，固定住dropout层和Batch Normalization层
    model.eval()
    is_coco = isinstance(data.get('val'), str) and data['val'].endswith('coco/val2017.txt')  # COCO dataset
    #确定检测的类别数目
    nc = 1 if single_cls else int(data['nc'])  # number of classes
    iouv = torch.linspace(0.5, 0.95, 10).to(device)  # iou vector for mAP@0.5:0.95
    #numel是pytorch预置函数，用来获取张量中的元素个数
    niou = iouv.numel()
    ...
    #训练时调用，需要加载数据集，验证时，直接从train.py中传入testloader
#5、初始化
    seen = 0
    #调用matrics中函数 存储混淆矩阵
   ...
   #6、验证过程
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        #四个变量分别代表batchsize、通道数量、图像高度、图像宽度
        ...
        # Inference
        #out是推理结果，1个
        #train_out是训练结果，3个[bs,anchor_num,grid_w,grid_h,xywh+c+20classes][1,3,80,80,25][1,3,40,40,25][1,3,20,20,25]
        out, train_out = model(im) if training else model(im, augment=augment, val=True)  # inference, loss outputs
        dt[1] += time_sync() - t2

        # Loss
        if compute_loss:
            #loss包含分类损失、置信度损失、边界框损失
        ...
        for si, pred in enumerate(out): # pred (tensor): (n, [xylsθ, conf, cls])
            #获取第si张图片的gt标签信息 包括class,x,y,w,h,theta
            ...
            if nl:
                ...
                #对当前的预测框与gt进行匹配，并且在预测框的对应位置上获取iou的评分信息
            ...
            #jdict字典保存的预测信息：imaged_id、category_id、bbox、score
    ...
    #6.6画出前面是三个batch图片的gt和pred框
        ...
        if plots and batch_i < 3:
         ...
         #创建子进程进行绘图，画出前三个batch图片的gt和pred框
    # Compute metrics
    '''
    计算评判分类效果的各种指标'''
